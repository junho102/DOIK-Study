module "efs_csi_driver_controller_iam_role" {
  ...
  role_name = "${local.eks_cluster_name}-AWSEFsCsiControllerIAMRole"
...
resource "aws_efs_file_system" "aws_efs_csi_driver_efs" {
  ...
  creation_token                  = "${local.eks_cluster_name}-efs-csi-driver"
  encrypted                       = "true"
...
resource "aws_efs_mount_target" "aws_efs_csi_driver_efs_mount_target" {
  count           = var.efs_csi_driver ? length(local.pri_subnets_ids) : 0
  file_system_id  = aws_efs_file_system.aws_efs_csi_driver_efs.0.id
  subnet_id       = element(local.pri_subnets_ids, count.index)
...
module "security-group-efs-csi-driver" {
  ...
  name                = "${local.eks_cluster_name}-efs-csi-driver-sg"
  vpc_id              = local.vpc_id
